...
function ReviewTransportation(props) {
  ...
  useEffect(() => {
    debugger;
    props.getTransportationById(localStorage.getItem("transportation-id"));
  }, []);
  ...
  const edit = (transportation) => {
    localStorage.setItem("transportation-id", transportation.id);
    window.location = "/edit-transportation";
  };
  ...
  return (
    <React.Fragment>
      ...
      {sessionStorage.getItem("roleWebShop") === "RegisteredUser" ? (
        ...
        props.transportation.ownerId ? (
        <div style={{ textAlign: "center" }} className="mt-5 pb-5">
          <button
            onClick={() => edit(props.transportation)}
            className="btn btn-primary"
          >
            Edit transportation
          </button>
